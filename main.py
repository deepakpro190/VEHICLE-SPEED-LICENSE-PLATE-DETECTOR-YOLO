...
# ✅ Configurations
VIDEO_INPUT = "vid1.mp4"  # Replace with your video file
VIDEO_OUTPUT = "output/21-03-2025_06.mp4"  # Output video file
...
vehicle_model = YOLO("yolov8n.pt")  # Use 'yolov8m.pt' for better accuracy
print("YOLOv8 Model loaded successfully.")

# ✅ Perspective Correction Setup
SOURCE = np.array([[320, 150], [960, 150], [1250, 720], [50, 720]])  # Pixel points
TARGET_WIDTH = 80
TARGET_HEIGHT = 50
TARGET = np.array([
    [0, 0],
...
def calculate_speed(track_id, new_x, new_y, fps):
    ...
    if transformed_point.size == 0:
        return 0  # No transformation available

    real_new_x, real_new_y = transformed_point[0]

    if track_id in prev_positions:
        old_x, old_y, old_time = prev_positions[track_id]
...
def run_speed_tracking():
    ...
    with tqdm(total=total_frames, desc="Processing Video", ncols=100) as pbar:
        while cap.isOpened():
            ...
            detections = results[0]  # First result
            ...
            if detections.boxes is not None:
                ...
                ids = range(len(boxes))  # Assign unique IDs for tracking
                ...
                for track_id, (x1, y1, x2, y2) in zip(ids, boxes):
                    ...
                    # Determine Box Color (Red if speed > 80 km/h)
                    box_color = (0, 255, 0) if speed <= 80 else (0, 0, 255)

                    # Draw Bounding Box and Speed
                    cv2.rectangle(frame, (int(x1), int(y1)), (int(x2), int(y2)), box_color, 2)
                    cv2.putText(frame, f"ID {track_id} | {int(speed)} km/h", (int(x1), int(y1) - 10), 
...
# ✅ Configurations
VIDEO_INPUT = "v2.webm"  # Input video file
VIDEO_OUTPUT = "output/21-03-2025_09.mp4"  # Output file
...
# ✅ Load YOLOv8 models
vehicle_model = YOLO("yolov8n.pt")  # For vehicle detection
plate_model = YOLO("runs/detect/trained12/weights/best.pt")  # For license plate detection
print("YOLO Models Loaded Successfully.")

# ✅ Set Tesseract OCR Path (Modify this path if necessary)
...
def extract_license_plate_text(plate_img):
    """Applies OCR on the detected license plate region."""
    ...
    with tqdm(total=total_frames, desc="Processing Video", ncols=100) as pbar:
        while cap.isOpened():
            ...
            if vehicle_detections.boxes is not None:
                boxes = vehicle_detections.boxes.xyxy.cpu().numpy()  # [x1, y1, x2, y2]
                ids = range(len(boxes))  # Assign unique IDs for tracking
            ...
            for plate in plate_results[0].boxes.xyxy.cpu().numpy():
                px1, py1, px2, py2 = map(int, plate)  # Plate coordinates

                plate_crop = frame[py1:py2, px1:px2]  # Extract plate region
                plate_text = extract_license_plate_text(plate_crop)

                # Draw plate box & display detected number
...
plate_model_path = hf_hub_download(repo_id="deepakpero190/my-yolo-plate-detect-model", filename="best.pt")
